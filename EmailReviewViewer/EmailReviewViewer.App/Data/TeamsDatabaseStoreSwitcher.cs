...
public sealed class TeamsDatabaseStoreSwitcher : IDisposable
{
    public TeamsRepository? Repository { get; private set; }
    ...
    public async Task SwitchAsync(string databasePath, CancellationToken cancellationToken = default)
    {
        ...
        var next = new TeamsRepository(fullPath);
    ...
    private static async Task ValidateAsync(string databasePath, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            await using (var command = connection.CreateCommand())
            {
                command.CommandText = """
                    SELECT name
                    FROM sqlite_master
                    WHERE name IN ('TeamsMessages', 'TeamsMessagesFts');
                    """;
            ...
            [
                ...
                "Preview", "BodyText", "MessageClass", "EntryId", "AttachmentsText",
                "ToRecipients", "CcRecipients"
            ], StringComparer.OrdinalIgnoreCase);
            await using (var command = connection.CreateCommand())
            {
                command.CommandText = "PRAGMA table_info(TeamsMessages);";
                await using var reader = await command.ExecuteReaderAsync(cancellationToken);
                while (await reader.ReadAsync(cancellationToken))
                    requiredColumns.Remove(reader.GetString(1));
            }

            if (!objects.Contains("TeamsMessages") ||
                !objects.Contains("TeamsMessagesFts") ||
                requiredColumns.Count > 0)
            {
                throw new InvalidDataException(
                    "The selected file is not a compatible Teams Review Viewer database.");
            }
        }
        ...
        {
            throw new InvalidDataException(
                $"The selected file is not a valid, readable Teams Review Viewer database. {exception.Message}",
                exception);
        }
    }
}
