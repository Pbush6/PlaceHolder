...
Describe 'Release artifact verification contracts' {
    ...
    It 'documents completed 1.3.1.0 version alignment without stale drift language' {
        $text = Get-Content -LiteralPath $script:contractsPath -Raw
        $text | Should -Match '\*\*Version:\*\*\s*1\.3\.1\.0'
        $text | Should -Match '1\.3\.1\.0'
